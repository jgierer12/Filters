...
# With help from @WireSegal or yrsegal and @abrightmoore
...
def perform(level, box, options):
	...
	if options["Step: "] == "Select":
		...
		if commandBlocks:
			...
			if options["How To Generate: "] == "@TwitchNitr0's Method" and options["Step: "] == "Generate":
			...
			elif options["How To Generate: "] == "Normal (Newer)" and options["Step: "] == "Generate":
				createCmdBlocks(level, box, options, commandBlocks)

			elif options["How To Generate: "] == "Normal (Old)" and options["Step: "] == "Generate":
				createCmdBlocksOld(level, box, options, commandBlocks)
...
def refactorCmdBlocks(level, box, options, commandBlocks): # abrightmoore - pure cmd block method
	...
	if options["Put Button To Start"] == True:

		level.setBlockAt(x, y+2, z+1, MARKERMATERIAL) # Block
		level.setBlockDataAt(x+1, y+2, z+1, GREEN) # Block Data
		level.setBlockAt(x-1, y+2, z+1, BUTTON)
		level.setBlockDataAt(x-1, y+2, z+1, BUTTOND)
	...
	for command in commandBlocks:
		...
		print(" ~ Generating Command Block At: " + '%s %s %s' % (x,y,z))
...
def createCmdBlocksOld(level, box, options, commandBlocks):
	...
	for command in commandBlocks:
		newPos = getNewPosOld(x, y, z, box, level, rsLen, direction)
		# newPos = getNewPos(level, box, stepNum)
		# stepNum = stepNum + 1
