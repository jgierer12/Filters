...
displayName = "Create Wireless Screen"
...
inputs = [
	...
	(
		("Input Block: ", "blocktype"),
		("I", "title"),
	),

	(
		("Output Block: ", "blocktype"),
		("II", "title"),
	),
...
def perform(level, box, options):
	...
	global inputProperties
	global outputProperties

	if options["Step: "] == "I":
		inputProperties = getProperties(level, box, options, "Input Block: ")
		if inputProperties[1] == 0:
			inputProperties = None
			raise Exception("The input block mustn't be air!")
	elif options["Step: "] == "II":
		if inputProperties:
			outputProperties = getProperties(level, box, options, "Output Block: ")

			if deltaPos(outputProperties[0]) != deltaPos(inputProperties[0]):
				outputProperties = None
				raise Exception("The output area must have exactly the same dimensions as the input area!")

			if outputProperties[1] == 0:
				outputProperties = None
				raise Exception("The output block mustn't be air!")

		else:
			raise Exception("Please select an input area first!")
	else:
		if inputProperties and outputProperties:
			...
			raise Exception("Please select an input and output area first!")

def getProperties(level, box, options, option):
	return ((box.minx, box.maxx, box.miny, box.maxy, box.minz, box.maxz), options[option].ID, options[option].blockData)

def createCommandBlocks(level, box, options):

	testforblockCommands = []
	setblockCommandsBlock = []
	setblockCommandsAir = []

	for x in xrange(inputProperties[0][0], inputProperties[0][1]):
		for y in xrange(inputProperties[0][2], inputProperties[0][3]):
			for z in xrange(inputProperties[0][4], inputProperties[0][5]):
				testforblockCommands.append("/testforblock "+str(x)+" "+str(y)+" "+str(z)+" "+getNewID(inputProperties[1])+" "+str(inputProperties[2]))

	for x in xrange(outputProperties[0][0], outputProperties[0][1]):
		for y in xrange(outputProperties[0][2], outputProperties[0][3]):
			for z in xrange(outputProperties[0][4], outputProperties[0][5]):
				setblockCommandsBlock.append("/setblock "+str(x)+" "+str(y)+" "+str(z)+" "+getNewID(outputProperties[1])+" "+str(outputProperties[2]))
				setblockCommandsAir.append("/setblock "+str(x)+" "+str(y)+" "+str(z)+" minecraft:air")
	...
	for testforblockCommand in testforblockCommands:
		...
		setblockCommandBlock = setblockCommandsBlock[i]
		setblockCommandAir = setblockCommandsAir[i]
		...
		level.setBlockAt(x+2, y, z, 137) # Command Block
		chunk = getChunk(x+2, z)
		commandBlock = cmdBlock(x+2, y, z, setblockCommandBlock)
		chunk.TileEntities.append(commandBlock)
		chunk.dirty = True
		...
		commandBlock = cmdBlock(x+3, y, z, setblockCommandAir)
		chunk.TileEntities.append(commandBlock)
		chunk.dirty = True
...
def deltaPos((minx, maxx, miny, maxy, minz, maxz)):
	deltaX = maxx-minx
	deltaY = maxy-miny
	deltaZ = maxz-minz
